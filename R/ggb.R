...
ggb <- function(S, g, type = c("global", "local"), lambda = NULL,
                delta = NULL, w = NULL, flmin = 0.01, nlam = 20,
  ...
  if (is.null(lambda)) {
    # this is a lambda that is large enough to ensure all V are 0
    # (w/o psd constraint)
  ...
  if (is.null(delta)) {
    if (type == "global")
      out <- ggb_global_nopsd(S, g, lambda = lambda, w = w)
    else if (type == "local")
      out <- ggb_local_nopsd(S, g, lambda = lambda, maxiter = in_iter,
                             tol = in_tol, verbose = verbose - 1)
    else stop("invalid type")
    out <- list(Sig = out$Sig, lambda = out$lambda, w = out$w,
                type = type, delta = delta, obj = out$obj)
    class(out) <- "ggb"
    return(out)
  }
  ...
  for (l in seq(nlam)) {
    ...
    for (i in seq(out_iter)) {
      ...
      R <- S + C
      if (type == "global") {
        out <- ggb_global_nopsd(R, g, lambda = lambda[l], w = w)
      } else if (type == "local") {
  ...
  out <- list(Sig = Sig, lambda = lambda, w = w, obj = obj)
  class(out) <- "ggb"
  out
}
