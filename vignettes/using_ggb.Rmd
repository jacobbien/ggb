...
```{r}
fit <- ggb(S, g, type = "global", flmin = 0.2)
```
...
We use the one standard error rule and get something that looks very similar to the true covariance matrix:
...
```{r}
fit2 <- ggb(S, g, type = "local", flmin = 0.2)
```
...
In this case, we get an estimate that resembles the true covariance matrix but is a little too sparse.
```{r,fig.show='hold'}
image_covariance(Sig, main = "True covariance")
image_covariance(Sighat2, main = "GGB estimate")
```
