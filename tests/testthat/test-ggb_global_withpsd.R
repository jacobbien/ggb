...
test_that("output beats Convex.jl", {
  lam <- 0.01
  del <- 0.5
  #cvx <- ggb_global_convexjl(S, lam, g = g, delta = del)
  fit1 <- ggb::ggb(S, g, type = "global", lambda = lam, delta = del,
                   out_tol = 1e-7)
  #expect_true(fit1$obj < cvx$obj)
  expect_true(fit1$obj < 0.2396776)
  expect_true(min(eigen(fit1$Sig[[1]], only.values = TRUE)$values) > del - tol)

  lam <- 0.1
  del <- 0.5
  #cvx <- ggb_global_convexjl(S, lam, g = g, delta = del)
  fit2 <- ggb::ggb(S, g, type = "global", lambda = lam, delta = del,
                   out_tol = 1e-7)
  #expect_true(fit2$obj < cvx$obj)
  expect_true(fit2$obj < 1.11051)
  expect_true(min(eigen(fit2$Sig[[1]], only.values = TRUE)$values) > del - tol)

  lam <- 0.2
  del <- 0.6
  #cvx <- ggb_global_convexjl(S, lam, g = g, delta = del)
  fit3 <- ggb::ggb(S, g, type = "global", lambda = lam, delta = del,
                   out_tol = 1e-7)
  #expect_true(fit3$obj < cvx$obj)
  expect_true(fit3$obj < 1.371911)
...
test_that("solving pathwise gives same results as one-at-a-time", {
  del <- 1
  fit <- ggb::ggb(S, g, type = "global", delta = del, nlam = 10,
